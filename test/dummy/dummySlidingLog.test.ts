...
import { DummySlidingLog } from "../../src/dummy/DummySlidingLog";

describe("DummySlidingLog", () => {
  let dummyRateLimiter: DummySlidingLog;

  beforeEach(() => {
    dummyRateLimiter = new DummySlidingLog();
  });

  describe("Constructor", () => {
    it("should create a dummy rate limiter without Redis", () => {
      const limiter = new DummySlidingLog();
      expect(limiter.getLimit()).toBe(-1);
      expect(limiter.getInterval()).toBe(-1);
    });

    it("should not throw errors during construction", () => {
      expect(() => new DummySlidingLog()).not.toThrow();
    });
  });
